...
export const getItemsHandler = withContract(GetItemsConfig)(async (
  req,
  res,
) => {
  ...
  const itemsData = Array.from(
    ...
    (_, i) => ({
      ...
      description: includeDetails
        ? `This is detailed description for item ${i + 1}.`
        : undefined,
      details: includeDetails
        ...
        : [],
      relatedItemIds:
        i % 2 === 0 ? [crypto.randomUUID(), crypto.randomUUID()] : [],
      // Extra field to demonstrate stripping by serialize
      internalNotes:
        "This note is for internal use only and should be stripped.",
    }),
  );
